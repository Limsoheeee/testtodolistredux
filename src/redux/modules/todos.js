...
const initialState = {
  todos: [
    { id: 0, title: "리액트", body: "리액트를 배워봅시다", isDone: false },
    { id: 1, title: "리액트", body: "리액트를 배워봅시다", isDone: true },
  ],
  todo: {},
};
...
const todos = (state = initialState, action) => {
  switch (action.type) {
    ...
    case DELETE_TODO:
      const filterDelCard = state.todos.filter((item) => {
        return item.id !== action.payload;
      });
      return { ...state, todos: filterDelCard };
